...
let Choise;
let fs = require("fs");
let pizzaFlavors = fs.existsSync("./saveFlavors.txt")
  ? JSON.parse(fs.readFileSync("./saveFlavors.txt"))
  : [];
let run = true;
...
function removeFlavor() {
  let rmv = parseInt(
    readlineSync.question(
      "Enter the number of the element you would like to remove: "
    )
  );
